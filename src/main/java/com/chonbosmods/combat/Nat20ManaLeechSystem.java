...
/**
 * Mana Leech: steal a percentage of damage dealt as mana.
 * Fires every hit, no proc chance. INT scaling.
 */
public class Nat20ManaLeechSystem extends DamageEventSystem {
    ...
    public Nat20ManaLeechSystem(Nat20LootSystem lootSystem) {
    ...
    public void handle(int entityIndex, ArchetypeChunk<EntityStore> chunk,
        ...
        for (RolledAffix rolledAffix : lootData.getAffixes()) {
            ...
            float restoreAmount = (float) (damage.getAmount() * effectivePercent);
            if (restoreAmount <= 0f) return;

            if (!statResolved) {
                manaIdx = EntityStatType.getAssetMap().getIndex("Mana");
                statResolved = true;
            }
            if (manaIdx < 0) return;
            ...
            if (CombatDebugSystem.isEnabled(attackerUuid)) {
                LOGGER.atInfo().log("[ManaLeech] player=%s leech=%.1f%% dmg=%.1f restore=%.2f",
                        attackerUuid.toString().substring(0, 8),
                        effectivePercent * 100, damage.getAmount(), restoreAmount);
            }
            return;
        }
    }
